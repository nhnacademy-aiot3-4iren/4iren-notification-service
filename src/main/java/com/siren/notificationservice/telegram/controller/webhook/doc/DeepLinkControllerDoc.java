...
public interface DeepLinkControllerDoc {
    ...
    ResponseEntity<LinkStatusResponse> getAdminLinkStatus(@Parameter(description = "Gateway가 JWT 검증 후 전달하는 유저 id", required = true) @RequestHeader("X-USER-ID") Long userId);
    ...
    ResponseEntity<LinkStatusResponse> getMemberLinkStatus(@Parameter(description = "Gateway가 JWT 검증 후 전달하는 유저 id", required = true) @RequestHeader("X-USER-ID") Long userId);
}
