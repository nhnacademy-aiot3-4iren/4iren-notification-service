...
public class FeedbackProcessingListener {
    ...
    public void handle(FeedbackProcessingEvent event) throws Exception {
        try{
            ...
            RoomEnvironmentReadingResponse readings = null;
            try{
                String referenceAtParam = referenceAt.format(DateTimeFormatter.ISO_OFFSET_DATE_TIME);
                readings = coreApiClient.getRoomEnvironments(event.roomId(), referenceAtParam);
            }catch (CoreApiUnavailableException e){
                /// core에서 환경 스냅샷을 못 받아와도 피드백 자체를 버리면 안됨 따라서 로그만 찍음
                log.warn("[FeedbackProcessingListener] 환경 실측값 조회 실패, 스냅샷 없이 저장 (roomId={}, referenceAt={})", event.roomId(), referenceAt, e);
            }
            feedbackPersistenceService.persist(event,referenceAt,readings);
        }catch (Exception e){
