...
class AlertHistoryRetentionServiceTest {
    ...
    void purgeDeletesInChunksUntilPartialPage() {
        ...
        when(alertHistoryRepository.deleteBatch(any(ZonedDateTime.class), eq(BATCH_SIZE)))
                .thenReturn(BATCH_SIZE, 500);

        retentionService.purge();

        verify(alertHistoryRepository, times(2)).deleteBatch(any(ZonedDateTime.class), eq(BATCH_SIZE));
    }
    ...
    void purgeStopsAfterOneCallWhenNothingToDelete() {
        ...
        verify(alertHistoryRepository, times(1)).deleteBatch(any(ZonedDateTime.class), eq(BATCH_SIZE));
    }
}
